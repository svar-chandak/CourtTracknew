...
import type { IndividualPositionMatch, Team } from '@/lib/team-match-types'

interface IndividualPositionScoreInputProps {
  match: IndividualPositionMatch
  homeTeam: Team
  awayTeam: Team
...
export function IndividualPositionScoreInput({ 
  match, 
  homeTeam,
  awayTeam,
  onScoreUpdate, 
  isReadOnly = false 
}: IndividualPositionScoreInputProps) {
  ...
  const completeMatch = (finalScore: MatchScore) => {
    ...
    toast.success(`${winner === 'home' ? homeTeam.school_name : awayTeam.school_name} wins!`)
    setIsLoading(false)
  }
  ...
  const getDivisionDisplayName = () => {
    ...
    return names[match.division] || match.division
  }
      ...
      <CardContent className="p-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">
              {getDivisionDisplayName()} - Position {match.position}
            </span>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="text-center">
            <div className="font-medium text-sm text-gray-600 mb-1">
              {homeTeam.school_name}
            </div>
            <div className="font-semibold text-lg">
              {getPlayerName(match.home_player1)}
            </div>
            {match.home_player2 && (
              <div className="text-sm text-gray-600">
                & {getPlayerName(match.home_player2)}
              </div>
            )}
          </div>
          ...
          <div className="text-center">
            <div className="font-medium text-sm text-gray-600 mb-1">
              {awayTeam.school_name}
            </div>
            <div className="font-semibold text-lg">
              {getPlayerName(match.away_player1)}
            </div>
            {match.away_player2 && (
              <div className="text-sm text-gray-600">
                & {getPlayerName(match.away_player2)}
              </div>
            )}
          </div>
        </div>
            ...
            {match.status === 'completed' && match.score && (
              <div className="text-center">
                ...
                <div className="text-sm text-green-600 font-medium">
                  Winner: {match.winner === 'home' ? homeTeam.school_name : awayTeam.school_name}
                </div>
              </div>
            )}
          </div>
        ...
        {!isStarted && (
          <div className="text-center">
            <Button
              onClick={startMatch}
              disabled={isReadOnly || !match.home_player1 || !match.away_player1}
              className="flex items-center gap-2"
            >
              <Play className="h-4 w-4" />
              Start Match
            </Button>
            {(!match.home_player1 || !match.away_player1) && (
